    ...
    import Header from '../../componentsPanel/HeaderPanel/HeaderPanel';
    ...
    const AdminPage = () => {
        ...
        // NOVA FUNÇÃO: Zerar Saldo do Afiliado
        const handleClearAffiliateBalance = (affiliate) => {
            Modal.confirm({
                title: `Zerar saldo de "${affiliate.name}"?`,
                icon: <DollarCircleOutlined />,
                content: `O saldo atual de ${parseFloat(affiliate.balance).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })} será zerado.`,
                okText: 'Sim, Zerar Saldo',
                okType: 'danger',
                cancelText: 'Cancelar',
                async onOk() {
                    try {
                        setLoading(prev => ({ ...prev, form: true })); // Usar loading do form para esta ação
                        await apiClient.put(`/admin/clients/${affiliate.id}/clear-balance`);
                        message.success(`Saldo de "${affiliate.name}" zerado com sucesso.`);
                        handleCancelModals(); // Fecha o modal
                        fetchAffiliates(); // Atualiza a lista de afiliados
                        fetchClients(pagination, filters); // Atualiza a lista de clientes para refletir o saldo
                    } catch (error) {
                        message.error(error.response?.data?.message || 'Falha ao zerar saldo do afiliado.');
                    } finally {
        ...
        const userTableColumns = [
            ...
            { title: 'Ações', key: 'actions', align: 'center', width: 80, fixed: 'right', render: (_, record) => (
                <Dropdown overlay={<Menu>
                        ...
                        {/* Exibe a opção de zerar saldo apenas se for um afiliado com saldo */}
                        {record.affiliateCode && record.balance > 0 && (
                            <Menu.Item key="4" icon={<DollarCircleOutlined />} onClick={() => handleClearAffiliateBalance(record)}>Zerar Saldo</Menu.Item>
                        )}
                        <Menu.Divider />
                        <Menu.Item key="5" icon={<DeleteOutlined />} danger onClick={() => handleDeleteUser(record)}>Excluir</Menu.Item>
                    </Menu>} trigger={['click']}>
        ...
        const affiliateTableColumns = [
            ...
            { title: 'Histórico', key: 'history', align: 'center', render: (_, record) => (
                <Space>
                    ...
                    {/* Botão de zerar saldo diretamente na tabela de afiliados */}
                    {record.balance > 0 && (
                        <Tooltip title="Zerar Saldo">
                            <Button type="primary" danger icon={<DollarCircleOutlined />} onClick={() => handleClearAffiliateBalance(record)} size="small" />
                        </Tooltip>
                    )}
                </Space>
            )},
        ...
        return (
            <div className="admin-page-content">
                ...
                <Modal open={modalState.details} onCancel={handleCancelModals} footer={
                    <div className="modal-footer-custom"> {/* NOVO: Footer customizado para detalhes */}
                        {viewingUser?.affiliateCode && viewingUser?.balance > 0 && (
                            <Button 
                                type="primary" 
                                danger 
                                icon={<DollarCircleOutlined />} 
                                onClick={() => handleClearAffiliateBalance(viewingUser)} 
                                size="large"
                                loading={loading.form}
                            >
                                Zerar Saldo (R${parseFloat(viewingUser.balance).toFixed(2).replace('.', ',')})
                            </Button>
                        )}
                        <Button onClick={handleCancelModals} size="large" className="cancel-btn-form">Fechar</Button>
                    </div>
                } className="admin-modal-white" width={600}>
                    {viewingUser && (
                        <div className="modal-content-wrapper user-details-modal-content">
                            ...
                            <Descriptions bordered layout="vertical" column={{ xs: 1, sm: 2 }}>
                                ...
                                {/* NOVO: Exibe o saldo de comissão se for um afiliado */}
                                {viewingUser.affiliateCode && (
                                    <Descriptions.Item label="Código de Afiliado">{viewingUser.affiliateCode}</Descriptions.Item>
                                )}
                                {typeof viewingUser.balance === 'number' && (
                                    <Descriptions.Item label="Saldo de Comissão">
                                        <Text strong style={{color: viewingUser.balance > 0 ? '#3f8600' : 'inherit'}}>
                                            {parseFloat(viewingUser.balance).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
                                        </Text>
                                    </Descriptions.Item>
                                )}
                            </Descriptions>
                ...
                <Modal 
                    ...
                    footer={
                        <div className="modal-footer-custom"> {/* NOVO: Footer customizado para detalhes do afiliado */}
                            {viewingAffiliate?.balance > 0 && (
                                <Button 
                                    type="primary" 
                                    danger 
                                    icon={<DollarCircleOutlined />} 
                                    onClick={() => handleClearAffiliateBalance(viewingAffiliate)} 
                                    size="large"
                                    loading={loading.form}
                                >
                                    Zerar Saldo (R${parseFloat(viewingAffiliate.balance).toFixed(2).replace('.', ',')})
                                </Button>
                            )}
                            <Button onClick={handleCancelModals} size="large" className="cancel-btn-form">Fechar</Button>
                        </div>
                ...
                >
                    {viewingAffiliate && (
                        <div className="modal-content-wrapper">
                            ...
                            {viewingAffiliate.referrals && viewingAffiliate.referrals.length > 0 ? (
                                <Table
                                ...
                                <Paragraph type="secondary">Este afiliado ainda não possui indicações registradas.</Paragraph>
                            )}
